//
// PUBLIC_INTERFACE
// exactTemplateExportService
// Loads the latest FP/T&M DOCX transcript .txt attachments from public/attachments,
// extracts placeholders, maps them to user-provided SOW form data, and generates a
// DOCX package that preserves the template layout while replacing only placeholder fields.
//
// This service strictly uses the provided template transcript files as the base,
// and only interpolates placeholder values. No alternate template is used.
//

/**
 * PUBLIC_INTERFACE
 * getLatestTemplateAttachmentPaths
 * Returns the public URLs to the most recent FP and TM transcript files to use for export/preview.
 * These must exist in public/attachments (copied by setup pipeline).
 */
export function getLatestTemplateAttachmentPaths() {
  // Prefer the most recent timestamped files if available, else fallback to earlier ones.
  // We keep explicit names so the CI can guarantee presence.
  return {
    FP: [
...
/**
 * PUBLIC_INTERFACE
 * loadTemplateTranscript
 * Load transcript text for a given template type ("FP" | "TM") using the latest available attachment.
 * Tries candidates in order and returns the first successful text.
 */
 ...
 * PUBLIC_INTERFACE
 * extractPlaceholders
 * Extract placeholder tokens from the transcript text.
 * Supports:
 * - [Field Name] tokens
 * - <Field Name> tokens
 * Returns a unique list in the order of appearance.
 */
export function extractPlaceholders(transcriptText) {
  ...
  for (const re of patterns) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      if (!seen.has(label)) {
        seen.add(label);
        out.push({ label, key: normalizeKey(label) });
      }
    }
  }
  return out;
...
/**
 * PUBLIC_INTERFACE
 * interpolateTranscript
 * Given a transcript and mapping function, replace placeholders in the text only.
 * Keeps all other characters, spacing, and layout intact.
 *
 * mapping: (label: string, key: string) => string | undefined
 */
export function interpolateTranscript(transcriptText, mapping) {
  const text = String(transcriptText || '');

  function replaceToken(full, inner, type) {
    const label = String(inner || '').trim().replace(/\s+/g, ' ');
    const key = normalizeKey(label);
    const val = mapping ? mapping(label, key) : undefined;
    const safe = val == null ? '' : String(val);
    // Return safe value; do not adjust surrounding spacing/newlines
    return safe;
  }
...
/**
 * PUBLIC_INTERFACE
 * generateExactDocxFromTranscript
 * Build a minimal .docx that preserves the transcript as paragraphs and merges user values in place.
 * This uses a straightforward conversion: each line => <w:p><w:t>line</w:t></w:p>
 * Thus the layout is governed by the original transcript structure.
 *
 * Note: We do not introduce custom headings or extra styles. Only a default Normal style is included.
 */
export function generateExactDocxFromTranscript(mergedTranscript, { logoDataUrl } = {}) {
  const lines = String(mergedTranscript || '').split(/\r?\n/);

  const media = {};
  const rels = [];
  let nextRid = 3; // rId1 styles reserved, rId2 numbering (unused)
  let logoRid = null;
 ...
 * PUBLIC_INTERFACE
 * zipSync
 * Minimal zip writer to package DOCX content without external deps.
 */
...
/**
 * PUBLIC_INTERFACE
 * mapUserDataForTemplate
 * Returns a mapping function usable by interpolateTranscript: looks up values in data.templateData
 * by normalized key first, and falls back to dotted paths if "authorization_signatures.*" etc.
 */
export function mapUserDataForTemplate(data) {
  const templateData = (data && data.templateData) || {};

  return (label, key) => {
    // Exact flat key lookup
    if (templateData[key] != null) return friendly(templateData[key]);
    // Special-case nested auth fields often present in our UI schema
    if (key.startsWith('supplier_signature_') || key.startsWith('client_signature_')) {
      const auth = templateData.authorization_signatures || {};
      if (auth[key]) return friendly(auth[key]);
      // Switch naming variants (e.g., supplier_signature_date vs supplier_date)
      const alt = key.replace(/^supplier_/, '').replace(/^client_/, '');
      if (auth[alt]) return friendly(auth[alt]);
    }
    // Try to match some common synonyms from parsed templates
    const altKey = tryResolveAltKey(key, templateData);
    if (altKey && templateData[altKey] != null) return friendly(templateData[altKey]);

    // Not found => empty (do not insert placeholder text)
    return '';
  };
}
...
function tryResolveAltKey(key, templateData) {
  // map a few known aliases based on parsed JSON keys
  const map = {
  ...
  // Also try nested authorization_signatures if present
  const auth = templateData.authorization_signatures || {};
